...
class ComplaintsCategoryController extends Controller
{
    ...
    {
        $this->middleware('permission:complaints_category.view|complaints_category.create|complaints_category.edit|complaints_category.delete|complaints_category.status_change')->only(['index', 'show']);
        $this->middleware('permission:complaints_category.create')->only(['store']);
        $this->middleware('permission:complaints_category.edit')->only(['edit', 'update']);
        $this->middleware('permission:complaints_category.delete')->only(['destroy']);
    }
    ...
    public function index(Request $request)
    {
        try {
            ...
            return view('admin::complaintsCategory.complaintsCategory', [
    ...
    public function show($id)
    {
        try {
            ...
            return view(
                'admin::complaints category.details',
    ...
    public function destroy($id)
    {
        try {
            ...
            // $category = ComplaintCategory::with(['complaints', 'serviceProviders'])->find($id);

            // // Check if the category has related complaints or service providers
            // if ($category->complaints()->exists() || $category->serviceProviders()->exists()) {
            //     // Redirect back with an error message
            //     return response()->json([
            //         'status' => 'error',
            //         'message' => 'Cannot delete because it has associated complaints or service providers',
            //     ]);
            // }
