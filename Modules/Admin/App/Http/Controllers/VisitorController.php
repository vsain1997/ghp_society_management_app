...
class VisitorController extends Controller
{
    ...
    {
        $this->middleware('permission:visitor.view')->only(['index', 'show', 'indexOtherVisitor', 'showOtherVisitor']);
    }
    ...
    public function indexNew(Request $request)
    {
        try {
            ...
            $datas = Visitor::with([
                ...
                ->select('visitors.id', 'visitors.user_id', 'visitors.type_of_visitor', 'visitors.visitor_name', 'visitors.phone', 'visitors.no_of_visitors', DB::raw('MAX(checkin_details.checkin_at)'), DB::raw('MAX(checkin_details.checkout_at)'))
                // ->select('visitors.*', 'checkin_details.checkin_at', 'checkin_details.checkout_at')
                ->join('checkin_details', 'visitors.id', '=', 'checkin_details.visitor_id')
                ->where('visitors.society_id', $selectedSociety)
                ->where('visitors.visitor_classification', 'resident_related');
            ...
            // Remove duplicates by using 'groupBy' on 'visitors.id' to ensure each visitor is unique
            $datas = $datas->groupBy('visitors.id', 'visitors.user_id', 'visitors.type_of_visitor', 'visitors.visitor_name', 'visitors.phone', 'visitors.no_of_visitors')
                // ->orderBy('visitors.id', 'desc')
    ...
    public function indexOtherVisitor(Request $request)
    {
        try {
            ...
            return view('admin::visitor.visitor-other', [
                'visitors' => $datas,
                'search' => $search,
                'societyResidents' => $societyResidents,
            ]);
            ...
            _dLog(eventType: 'error', activityName: 'Visitor (Other) Index Error', description: 'Exception during visitor (Other) index retrieval: ' . $e->getMessage(), status: 'failed', severityLevel: 2);
            return redirect()->back()->with([
                'status' => 'error',
                'message' => 'Failed, please try again!',
            ]);
    ...
    public function showOtherVisitor($id)
    {
        try {
            ...
            return view(
                'admin::visitor.details-other',
                compact('visitor', 'checkinDetails')
            );
        } catch (Exception $e) {

            _dLog(eventType: 'error', activityName: 'Visitor (Other) Details Error', description: 'Exception during visitor (Other) details retrieval: ' . $e->getMessage(), modelType: 'Visitor', modelId: null, status: 'failed', severityLevel: 2);
