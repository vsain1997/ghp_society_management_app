...
class EventController extends Controller
{
    ...
    {
        $this->middleware('permission:event.view|event.create|event.edit|event.delete')->only(['index', 'show']);
        $this->middleware('permission:event.create')->only(['store']);
        $this->middleware('permission:event.edit')->only(['edit', 'update']);
        $this->middleware('permission:event.delete')->only(['destroy']);
        // $this->middleware('permission:event.status_change')->only(['changeStatus']);
    ...
    public function store(Request $request)
    {
        try {
            ...
            $convertedTime = Carbon::createFromFormat('h:i A', $request->input('time'), 'Asia/Kolkata')->format('H:i:s');
            ...
            $event->time = $convertedTime;//$request->input('time');
            $event->image = $path;
            ...
            _dLog(eventType: 'info', activityName: 'Event Created', description: 'New event created', modelType: 'Event', modelId: $event->id, status: 'success', severityLevel: 1, beforeData: null, afterData: $event->toArray(), requestData: $request->all());
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $event->time = $convertedTime;//$request->time;
