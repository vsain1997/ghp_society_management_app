...
class DocumentController extends Controller
{
    ...
    {
        $this->middleware('permission:document.request|document.upload_send|document.view')->only(['index', 'show']);
        $this->middleware('permission:document.request')->only(['store']);
        $this->middleware('permission:document.upload_send')->only(['uploadDocument']);
    }
    ...
    public function index(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                'request_type' => 'nullable|string|in:residents_request,management_request,ownself_request',
            ]);
            ...
            if ($request_type == 'residents_request') {
                ...
                //[not showing admin role based app user requests here]
                $query->where('request_by_role', 'resident');

            } elseif ($request_type == 'ownself_request') {
                //show residents request
    ...
    public function show($id)
    {
        try {
            ...
            if ($document->request_by_role == 'resident') {
                ...
                $residentOtherInfo = Member::select('members.user_id', 'members.name', 'members.aprt_no', 'members.floor_number', 'members.unit_type', 'members.phone', 'blocks.name as block_name')
                    ...
                    ->first();
            } elseif ($document->request_by_role == 'admin' || $document->request_by_role == 'super_admin') {
