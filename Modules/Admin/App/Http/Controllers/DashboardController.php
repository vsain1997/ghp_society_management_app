...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $societyDetails = Society::find($society_id);
            $societyName = $societyDetails->name;
            // total resident unit
            // $society = Society::where('status', 'active')->where('id', $society_id)->first();
            // $society_units = (int) $society->floors * (int) $society->floor_units;
            $blocks = Block::where('society_id', $society_id)->count();
            $society_units = $blocks;
            // $society = Society::with('blocks')
            //     ->where('id', $society_id)
            //     ->first(); // Fetch the single society with id = 5
            ...
            // // Now $society contains the single society with total_society_units calculated


            // $society_units = (int) $society->total_society_units;
