...
@section('content')
    <div class="right_main_body_content members_page">
        <div class="head_content">
            ...
            @can('service_provider_category.create')
                <button type="button" class="bg_theme_btn" id="addSproviderCategoryModalOpen" data-bs-toggle="modal"
        ...
        <div class="custom_table_wrapper">
            <div class="filter_table_head">
                ...
                <div class="right_filters">
                    <h2>Service Category Listing </h2>
                    {{-- <button class="sortby">
            ...
            <div class="table-responsive">
                <table width="100%" cellpadding="0" cellspacing="0">
                    ...
                    <tbody>
                        @php
                            $sl = 0;
                        @endphp
                        @if ($sproviderCategory && !$sproviderCategory->isEmpty())
                            @foreach ($sproviderCategory as $sCategory)
                                @php
                                    $sl++;
                                @endphp
                                <tr>
                                    <td>{{ $sl }}</td>
                                    <td class="event-user"><img src="{{ $sCategory->image }}" alt="">
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <div class="actions">
                                            @can('service_provider_category.edit')
                                            ...
                                            @can('service_provider_category.delete')
                ...
                <div class="table_bottom_box">
                    ...
                    <div class="d-flex justify-content-between p-2 mt-2 mb-2">
                        <div>
                            Showing {{ $sproviderCategory->firstItem() }} to {{ $sproviderCategory->lastItem() }} of
                            {{ $sproviderCategory->total() }} results
                        </div>
                        <div>
                            {{ $sproviderCategory->links('vendor.pagination.bootstrap-5') }} {{-- Bootstrap 5 pagination view --}}
                        </div>
                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content ">
                <div class="modal-header">
                    <h3 class="modal-title text-white fs-5" id="modalHeadTxt">Add Service Category</h3>
                    {{-- <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button> --}}
                </div>
                <div class="modal-body">
                    <div class="custom_form">
                        <form method="POST" action="{{ route($thisModule . '.service_provider_category.store') }}"
                            id="addSproviderCategoryForm" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="save-close-btn">
                                ...
                                <button type="button" data-formtype="add" id="submitAddSproviderCategoryForm"
...
@push('footer-script')
    <script>
        $(".members_page").on('click', '#addSproviderCategoryModalOpen', function(event) {
            // set modal form
            $('#addSproviderCategoryForm').attr('action',
                '{{ route($thisModule . '.service_provider_category.store') }}'
            );
            $('#submitAddSproviderCategoryForm').attr('data-formtype', 'add');
            $('#modalHeadTxt').text('Add Service Category');
            $('#submitAddSproviderCategoryForm').text('Submit');

            // reset form data
            $('#addSproviderCategoryForm').find(
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".modal").on('click', '#submitAddSproviderCategoryForm', async function(event) {
                ...
                // let formType = $('#submitAddSproviderCategoryForm').data('formtype');
                let formType = $('#id').val();
                ...
                if (formType == 'add') {
                    ...
                    $('#addSproviderCategoryForm').submit();
                } else {
                    //on modal-cancel relaod page to show fresh updated data
                    $('#addSproviderCategoryForm').find('.cancel_btn').attr('onclick',
                        'window.location.reload()');
                    // let formData = $('#addSproviderCategoryForm').serialize();
                    let formData = new FormData($('#addSproviderCategoryForm')[0]);
                    $.ajax({
                        url: $('#addSproviderCategoryForm').attr('action'),
                        method: 'POST',
                        data: formData,
                        headers: {
    ...
    <script>
        $(document).ready(function() {
            $('body').on('click', '.edit', function() {
                ...
                // $('#addSproviderCategoryForm')[0].reset();
                $('#addSproviderCategoryForm').find(
                ...
                $('#addSproviderCategoryForm select').each(function() {
                ...
                $('#modalHeadTxt').text('Edit Service Provider Category');
                $('#submitAddSproviderCategoryForm').text('Update');

                const complaintCategoryId = $(this).attr('id');

                $.ajax({
                    url: "{{ route($thisModule . '.service_provider_category.edit', ['id' => ':complaintCategoryId']) }}"
                    ...
                    success: function(res) {
                        ...
                        $('#addSproviderCategoryForm').attr('action',
                            '{{ route($thisModule . '.service_provider_category.update', ['id' => '__ID__']) }}'
                            .replace('__ID__', data.id));
                        $('#submitAddSproviderCategoryForm').attr('data-formtype', 'edit');

                        //feed #addSproviderCategoryForm form data by jq below
                        $('#id').val(data.id);
                        $('#title').val(data.name);
                        //loader removed
                        $('#loader').css('width', '100%');
            ...
            $('.delete').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route($thisModule . '.service_provider_category.delete', ['id' => ':complaintCategoryId']) }}"
                                .replace(':complaintCategoryId', complaintCategoryId),
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': csrfToken
