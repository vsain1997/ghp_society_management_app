...
@section('title', 'Complaints Category')
...
@section('content')
    <div class="right_main_body_content members_page">
        <div class="head_content">
            ...
            @can('complaints_category.create')
                <button type="button" class="bg_theme_btn" id="addComplaintsCategoryModalOpen" data-bs-toggle="modal"
        ...
        <div class="custom_table_wrapper">
            <div class="filter_table_head">
                ...
                <div class="right_filters">
                    <h2>Complaints Category Listing </h2>
                    {{-- <button class="sortby">
            ...
            <div class="table-responsive">
                <table width="100%" cellpadding="0" cellspacing="0">
                    ...
                    <tbody>
                        @php
                            $sl = 0;
                        @endphp
                        @if ($complaintsCategory && !$complaintsCategory->isEmpty())
                            @foreach ($complaintsCategory as $cCategory)
                                @php
                                    $sl++;
                                @endphp
                                <tr>
                                    ...
                                    <td class="text-center">
                                        {{ \Carbon\Carbon::parse($cCategory->created_at)->format('d M Y') }}</td>
                                    <td class="text-center">
                                        <div class="actions">
                                            @can('complaints_category.edit')
                                            ...
                                            @can('complaints_category.delete')
                ...
                <div class="table_bottom_box">
                    ...
                    <div class="d-flex justify-content-between p-2 mt-2 mb-2">
                        <div>
                            Showing {{ $complaintsCategory->firstItem() }} to {{ $complaintsCategory->lastItem() }} of
                            {{ $complaintsCategory->total() }} results
                        </div>
                        <div>
                            {{ $complaintsCategory->links('vendor.pagination.bootstrap-5') }} {{-- Bootstrap 5 pagination view --}}
                        </div>
                    </div>
    ...
    <!-- Modal -->
    <div class="modal fade custom_Modal" id="addComplaintCategoryModal" tabindex="-1"
        aria-labelledby="addComplaintCategoryModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content ">
                <div class="modal-header">
                    <h3><span id="modalHeadTxt">Add Complaint Category</span></h3>
                    {{-- <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button> --}}
                </div>
                <div class="modal-body">
                    <div class="custom_form">
                        <form method="POST" action="{{ route($thisModule . '.complaints_category.store') }}"
                            id="addComplaintsCategoryForm">
                            @csrf
                            <div class="row">
                            ...
                            <div class="save-close-btn">
                                ...
                                <button type="button" data-formtype="add" id="submitAddComplaintsCategoryForm"
...
@push('footer-script')
    <script>
        $(".members_page").on('click', '#addComplaintsCategoryModalOpen', function(event) {
            // set modal form
            $('#addComplaintsCategoryForm').attr('action',
                '{{ route($thisModule . '.complaints_category.store') }}'
            );
            $('#submitAddComplaintsCategoryForm').attr('data-formtype', 'add');
            $('#modalHeadTxt').text('Add Complaint Category');
            $('#submitAddComplaintsCategoryForm').text('Submit');

            // reset form data
            $('#addComplaintsCategoryForm').find(
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".modal").on('click', '#submitAddComplaintsCategoryForm', async function(event) {
                ...
                // let formType = $('#submitAddComplaintsCategoryForm').data('formtype');
                let formType = $('#id').val();
                ...
                if (formType == 'add') {
                    ...
                    $('#addComplaintsCategoryForm').submit();
                } else {
                    //on modal-cancel relaod page to show fresh updated data
                    $('#addComplaintsCategoryForm').find('.cancel_btn').attr('onclick',
                        'window.location.reload()');
                    let formData = $('#addComplaintsCategoryForm').serialize();
                    $.ajax({
                        url: $('#addComplaintsCategoryForm').attr('action'),
                        method: 'POST',
                        data: formData,
                        headers: {
    ...
    <script>
        $(document).ready(function() {
            $('body').on('click', '.edit', function() {
                ...
                // $('#addComplaintsCategoryForm')[0].reset();
                $('#addComplaintsCategoryForm').find(
                ...
                $('#addComplaintsCategoryForm select').each(function() {
                ...
                $('#modalHeadTxt').text('Edit Complaint Category');
                $('#submitAddComplaintsCategoryForm').text('Update');

                const complaintCategoryId = $(this).attr('id');

                $.ajax({
                    url: "{{ route($thisModule . '.complaints_category.edit', ['id' => ':complaintCategoryId']) }}"
                    ...
                    success: function(res) {
                        ...
                        $('#addComplaintsCategoryForm').attr('action',
                            '{{ route($thisModule . '.complaints_category.update', ['id' => '__ID__']) }}'
                            .replace('__ID__', data.id));
                        $('#submitAddComplaintsCategoryForm').attr('data-formtype', 'edit');

                        //feed #addComplaintsCategoryForm form data by jq below
                        $('#id').val(data.id);
                        $('#title').val(data.name);
                        //loader removed
                        $('#loader').css('width', '100%');
            ...
            $('.delete').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route($thisModule . '.complaints_category.delete', ['id' => ':complaintCategoryId']) }}"
                                .replace(':complaintCategoryId', complaintCategoryId),
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': csrfToken
