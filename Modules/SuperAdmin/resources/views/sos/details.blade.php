        ...
        <div class="bg-card">
            <div class="society_details">
                <ul>
                    <li>
                        <p>Society</p>
                        <h4>{{ $sos->society->name }}</h4>
                    </li>
                    <li>
                        <p>From</p>
                        <h4>
                            @if ($sos->user->role == 'admin')
                                {{ 'Society Admin' }}
                            @elseif ($sos->user->role == 'resident')
                                {{ 'Resident' }}
                            @elseif ($sos->user->role == 'staff_security_guard')
                                {{ 'Security Guard' }}
                            @endif
                        </h4>
                    </li>
                    ...
                    @if (isset($sos->block_id))
                        <li>
                            <p>Tower </p>
                            <h4>{{ $sos->block->name }}</h4>
                        </li>
                        <li>
                            <p>Floor </p>
                            <h4>{{ $sos->floor }}</h4>
                        ...
                        <li>
                            <p>Property Type </p>
                            <h4>{{ $sos->block->unit_type }}</h4>
                        </li>
                    @endif
                    <li>
                        <p>Date</p>
                        <h4>{{ \Carbon\Carbon::parse($sos->date)->format('d M Y') }}</h4>
                    </li>
                    <li>
                        <p>Time</p>
                        <h4>{{ \Carbon\Carbon::parse($sos->time)->format('g:i A') }} </h4>
                    </li>
                    <li>
                        <p>Status </p>
                        <h4>{{ Str::ucfirst($sos->status) }}</h4>
                    </li>
                    @if ($sos->status =='acknowledged')
                        <li>
                            <p>Acknowledged  By ( Security Guard )</p>
                            <h4>{{ Str::ucfirst($sos->acknowledgedBy->name) }}</h4>
                        </li>
                    @endif
                </ul>
                ...
                <br>
                <p>Description</p>
                <p style="text-transform: none">{{ $sos->description }}</p>
            </div>
        </div>
    </div>
@endsection
